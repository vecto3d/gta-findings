		...
		switch (iLocal_267)
		{
			...
			case 9:
				...
				if (!bVar37 && (!bVar36 || bLocal_454))
				{
					if (bLocal_524)
					{
						Global_114908[iLocal_268] = uLocal_523;
					}
					else
					{
...
void func_1()
{
	if (!Global_4 || iLocal_467)
	{
		return;
	}
	if ((ENTITY::DOES_ENTITY_EXIST(iLocal_465) && !ENTITY::IS_ENTITY_DEAD(iLocal_465, false)) && !ENTITY::IS_ENTITY_DEAD(iLocal_257, false))
	{
		if (!iLocal_466)
		{
			if (MISC::GET_DISTANCE_BETWEEN_COORDS(ENTITY::GET_ENTITY_COORDS(iLocal_257, true), ENTITY::GET_ENTITY_COORDS(PLAYER::PLAYER_PED_ID(), true), true) < 15f)
			{
				...
				TASK::TASK_FOLLOW_POINT_ROUTE(iLocal_465, 1f, 0);
				iLocal_466 = 1;
			}
		}
		else
		{
			PED::SET_IK_TARGET(iLocal_465, 1, iLocal_257, 0, 0f, 0f, 0f, 0, -1, -1);
			if (MISC::GET_DISTANCE_BETWEEN_COORDS(ENTITY::GET_ENTITY_COORDS(iLocal_465, true), 377.4565f, 154.6708f, 102.0838f, true) > 25f)
			{
				ENTITY::SET_PED_AS_NO_LONGER_NEEDED(&iLocal_465);
				iLocal_467 = 1;
			}
			else if (MISC::GET_DISTANCE_BETWEEN_COORDS(ENTITY::GET_ENTITY_COORDS(iLocal_465, true), ENTITY::GET_ENTITY_COORDS(iLocal_257, true), true) < 3f && iLocal_468 == 0)
			{
				iLocal_468 = 1;
			}
		}
...
void func_9()
{
	...
	{
		AUDIO::REMOVE_ENTITY_FROM_AUDIO_MIX_GROUP(iLocal_257, 0f);
	}
}
...
void func_105()
{
	...
	iLocal_465 = PED::CREATE_PED(4, joaat("a_m_m_business_01"), 396.8498f, 143.3195f, 101.4659f, 360f, true, true);
	if ((ENTITY::DOES_ENTITY_EXIST(iLocal_465) && !ENTITY::IS_ENTITY_DEAD(iLocal_465, false)) && !ENTITY::IS_ENTITY_DEAD(iLocal_257, false))
	{
		PED::SET_IK_TARGET(iLocal_465, 1, iLocal_257, 0, 0f, 0f, 0f, 0, -1, -1);
	}
	if (!ENTITY::IS_ENTITY_DEAD(iLocal_257, false))
	{
		AUDIO::ADD_ENTITY_TO_AUDIO_MIX_GROUP(iLocal_257, "MAG_2_PAM_PED_GROUP", 0f);
	}
}

void func_106()
{
	iLocal_53 = joaat("u_f_o_moviestar");
	StringCopy(&Local_251, "PBPD", 8);
	StringCopy(&Local_253, "PAMELADRAKE", 16);
	StringCopy(&Local_219, "Special_Ped@pamela", 64);
	StringCopy(&Local_235, "Base", 64);
	bLocal_453 = true;
	iLocal_269 = 14;
	sLocal_320 = "PBPD_INTERACT";
	StringCopy(&Local_329, "PBPD_CONV_GM", 16);
	StringCopy(&Local_333, "PBPD_CONV_GT", 16);
	StringCopy(&Local_337, "PBPD_CONV_GF", 16);
	StringCopy(&cLocal_341, "special_ped@pamela@base", 64);
	StringCopy(&cLocal_357, "special_ped@pamela@intro", 64);
	iLocal_268 = 10;
	iLocal_272[0] = 8;
	iLocal_272[1] = 8;
	iLocal_272[2] = 8;
...
void func_114(int iParam0)
{
	...
	if (iLocal_258)
	{
		...
		if (MISC::ARE_STRINGS_EQUAL(&uVar0, &Local_519))
		{
			func_13();
		}
		if (iParam0 == 1)
